			...
			>
				<view class="base-scroll-list-columns base-scroll-list-text">
					...
					<view style="display: flex;flex-direction: row;width: 100%;">
						<view class="base-text" style="width: 15%;":style="{ color: item.CompleteState =='已延期' ? 'red' : 'white' }">{{item.OrderNum}}</view>
						<view class="base-text"style="width: 10%;":style="{ color: item.CompleteState =='已延期' ? 'red' : 'white' }">{{item.ShortChar02}}</view>
						<view class="base-text"style="width: 15%":style="{ color: item.CompleteState =='已延期' ? 'red' : 'white' }">{{item.ShortChar03}}</view>
						<view class="base-text"style="width:10%;":style="{ color: item.CompleteState =='已延期' ? 'red' : 'white' }">{{item.OrderDate}}</view>
						<view class="base-text"style="width: 30%":style="{ color: item.CompleteState =='已延期' ? 'red' : 'white' }">{{item.Machine}}</view>
						<view class="base-text"style="width: 5%;":style="{ color: item.CompleteState =='已延期' ? 'red' : 'white' }">{{item.Qty}}</view>
						<view class="base-text" style="width: 12.5%;":style="{ color: item.CompleteState =='已延期' ? 'red' : 'white' }">{{item.PlanDate}}</view>
						<view class="base-text" style="width: 12.5%;":style="{ color: item.CompleteState =='已延期' ? 'red' : 'white' }">{{item.CompletePre}}</view>
					</view>
	...
	export default {
		...
		methods: {
			updateChange() {
				...
				console.log(this.rowsData.length,'this.rowsData.length') 
				...
				console.log(this.avgHeight,'this.avgHeight') 
				  // 展示动画
